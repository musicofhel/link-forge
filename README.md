...
Personal knowledge forager — drop links into a Discord channel, an agent scrapes/categorizes/embeds them into a Neo4j graph database, and an MCP server lets any Claude Code session query the collected knowledge.
...
```
Discord → Bot → SQLite Queue → Processor → Neo4j Graph ← MCP Server ← Claude Code
```

- **Scraping**: fetch + @mozilla/readability
- **Categorization**: Claude Code CLI (`claude -p`)
